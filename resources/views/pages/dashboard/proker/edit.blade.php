    ...
    <form method="POST" action="{{ route('dashboard.proker.update', $formState['id']) }}" enctype="multipart/form-data"
                ...
                <template x-for="(label, i) in stepLabels" :key="i">
                    <div class="flex items-center" :class="i < stepLabels.length - 1 ? 'flex-1' : ''">
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-[10px] font-medium mt-1.5 text-center max-w-20 leading-tight"
                                :class="i + 1 <= step ? 'text-[#2C3DA6]' : 'text-gray-400'" x-text="label"></span>
                        </div>

                        <template x-if="i < stepLabels.length - 1">
                            <div class="flex-1 h-0.5 mx-2 -mt-4"
        ...
        <div x-show="step === 1" class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <label class="cursor-pointer">
                        <input type="radio" name="risk_level" value="rendah" class="sr-only peer"
                            @checked(old('risk_level', $formState['risk_level']) === 'rendah')>
                        <div
                            class="p-3 text-center rounded-xl border-2 border-gray-200 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 text-gray-600 peer-checked:text-emerald-700 font-semibold text-xs transition-all">
                            🟢 Rendah
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        <input type="radio" name="risk_level" value="sedang" class="sr-only peer"
                            @checked(old('risk_level', $formState['risk_level']) === 'sedang')>
                        <div
                            class="p-3 text-center rounded-xl border-2 border-gray-200 peer-checked:border-amber-500 peer-checked:bg-amber-50 text-gray-600 peer-checked:text-amber-700 font-semibold text-xs transition-all">
                            🟡 Sedang
                        </div>
                    </label>
                    <label class="cursor-pointer">
                        <input type="radio" name="risk_level" value="tinggi" class="sr-only peer"
                            @checked(old('risk_level', $formState['risk_level']) === 'tinggi')>
                        <div
                            class="p-3 text-center rounded-xl border-2 border-gray-200 peer-checked:border-red-500 peer-checked:bg-red-50 text-gray-600 peer-checked:text-red-700 font-semibold text-xs transition-all">
                            🔴 Tinggi
                        </div>
                    </label>
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" placeholder="Jelaskan gambaran umum kegiatan..."
                    class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:outline-none focus:border-[#2C3DA6] focus:ring-2 focus:ring-[#2C3DA6]/20 transition-all">{{ old('description', $formState['description']) }}</textarea>
            </div>

            {{-- Pengaturan Publikasi Event --}}
            <div class="pt-4 border-t border-gray-100">
                ...
                <div class="mb-4">
                    ...
                    @if ($prokerRow?->poster)
                        <div class="mb-2">
                            <img src="{{ \App\Services\StorageHelper::url($prokerRow->poster) }}" alt="Poster" class="h-32 rounded-lg object-cover border border-gray-200" loading="lazy">
                ...
                <div class="space-y-3">
                    <template x-for="(m, i) in milestones" :key="i">
                        <div class="flex items-center gap-3">
                            <span
                                class="w-7 h-7 rounded-full bg-[#2C3DA6]/10 text-[#2C3DA6] text-xs font-bold flex items-center justify-center shrink-0"
                                x-text="i + 1"></span>
