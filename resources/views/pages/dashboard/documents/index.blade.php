<x-layouts.dashboard title="Dokumentasi">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6" x-data="{ uploadOpen: false }">
        <div>
    ...
    @php
        $iconColors = [
            ...
            'ppt' => ['bg' => 'bg-orange-50', 'text' => 'text-orange-500', 'icon' => 'PPT'],
            'other' => ['bg' => 'bg-gray-50', 'text' => 'text-gray-500', 'icon' => 'FILE'],
        ];
    @endphp
    ...
    @if ($documents->isNotEmpty())
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
            @foreach ($documents as $doc)
                ...
                @endphp
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group flex flex-col justify-between">
                    <div>
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-gray-50">
                        ...
                        <div class="flex items-center gap-1">
                            <a href="{{ route('dashboard.documents.download', $doc->id) }}" class="p-1.5 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-[#2C3DA6] transition-colors" title="Download Dokumen">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            </a>
                            <form method="POST" action="{{ route('dashboard.documents.destroy', $doc->id) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus arsip file ini?');" class="inline">
                                @csrf
                                @method('DELETE')
