<x-layouts.public title="Event & Proker" description="Daftar event dan program kerja HMSE yang terbuka untuk umum">
    ...
    {{-- Page Header --}}
    <section class="pt-28 pb-14 bg-linear-to-br from-[#0f2044] via-hmse-primary to-hmse-secondary relative overflow-hidden">
        {{-- Decorative circles --}}
        <div class="absolute top-0 right-0 w-96 h-96 bg-hmse-secondary/20 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl pointer-events-none"></div>
        <div class="absolute bottom-0 left-0 w-64 h-64 bg-hmse-accent/10 rounded-full translate-y-1/2 -translate-x-1/4 blur-2xl pointer-events-none"></div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            <span class="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 text-white/80 text-xs font-semibold rounded-full mb-5 backdrop-blur-sm border border-white/10">
                <svg class="w-3.5 h-3.5 text-hmse-accent" fill="currentColor" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                Program Kerja HMSE
            </span>
    ...
    <section class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('events.index') }}"
                ...
                <div class="relative flex-1">
                    ...
                    <input
                        ...
                        class="pl-10 pr-4 py-2.5 w-full text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:border-hmse-secondary focus:ring-2 focus:ring-hmse-secondary/20 transition-all duration-200"
                    >
                </div>

                {{-- Division Filter --}}
                <select name="division"
                        class="px-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:border-hmse-secondary focus:ring-2 focus:ring-hmse-secondary/20 transition-all duration-200 min-w-45">
                ...
                {{-- Status Filter --}}
                <select name="status"
                        class="px-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:border-hmse-secondary focus:ring-2 focus:ring-hmse-secondary/20 transition-all duration-200 min-w-37.5">
                ...
                <button type="submit"
                        class="px-6 py-2.5 bg-hmse-primary text-white text-sm font-semibold rounded-xl hover:bg-hmse-primary-light transition-colors duration-200">
                    Cari
                </button>
            ...
            <p class="text-sm text-gray-500 mb-6">
                Menampilkan <span class="font-semibold text-hmse-primary">{{ $events->total() }}</span> event
                        ...
                        <article class="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col">
                            ...
                            <a href="{{ route('events.show', $event->id) }}" class="block relative overflow-hidden">
                                <div class="aspect-video bg-gray-100">
                                    @if($event->poster)
                                        <img
                                            src="{{ \App\Services\StorageHelper::url($event->poster) }}"
                                ...
                                @if($isOpen)
                                    <span class="absolute top-3 right-3 px-2.5 py-1 bg-hmse-primary text-white text-xs font-bold rounded-full shadow-sm animate-pulse">
                                        Buka Daftar
                                    </span>
                                @elseif($isFull)
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                ...
                                {{-- Division --}}
                                <p class="text-xs font-semibold text-hmse-secondary uppercase tracking-wider mb-1.5">
                                    {{ $event->division }}
                                </p>

                                {{-- Title --}}
                                <h2 class="text-base font-bold text-hmse-primary leading-snug mb-3 line-clamp-2 flex-1">
                                    <a href="{{ route('events.show', $event->id) }}" class="hover:text-hmse-secondary transition-colors duration-200">
                                        {{ $event->name }}
                                    </a>
                                </h2>
                                ...
                                <div class="space-y-1.5 mb-4">
                                    <div class="flex items-center gap-2 text-xs text-gray-500">
                                        <svg class="w-3.5 h-3.5 text-hmse-secondary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('events.show', $event->id) }}"
                                   ...
                                   {{ $isOpen
                                       ? 'bg-hmse-primary text-white hover:bg-hmse-primary-light'
            ...
            @else
                ...
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    @if($search || $division || $status)
                        <a href="{{ route('events.index') }}"
                           class="px-5 py-2.5 bg-hmse-primary text-white text-sm font-semibold rounded-xl hover:bg-hmse-primary-light transition-colors duration-200">
                            Lihat Semua Event
                        </a>
                    @endif
                </div>
