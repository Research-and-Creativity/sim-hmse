@php
    ...
    $isAdmin = $role === 'admin';
    $isPengurus = in_array($role, ['admin', 'pengurus']);
@endphp
...
>
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 overflow-hidden">
            ...
            <div x-show="sidebarOpen" x-transition:enter="transition-opacity duration-200" class="overflow-hidden">
                ...
                <span class="text-[10px] text-gray-400 leading-none whitespace-nowrap">
                    {{ $isAdmin ? 'Super Admin' : 'Pengurus HIMA' }}
                </span>
            </div>
        </a>
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        {{-- Switch Portal for Super Admin --}}
        @if($isAdmin)
            <div class="my-3 border-t border-gray-100"></div>
            <p x-show="sidebarOpen" class="px-3 mb-2 text-[10px] font-bold text-cyan-600 uppercase tracking-widest">Portal Khusus</p>
            <x-dashboard.sidebar-link
                href="{{ route('pembina.dashboard') }}"
                :active="request()->routeIs('pembina.*')"
